	...
	public static void main(String args[]) throws EncryptedDocumentException, InvalidFormatException, IOException,
		...
		if (jedis.ping().equalsIgnoreCase("PONG")) {
			...
			for (String writeexternal : indivseeds) {
				// Right pushing and storing all the data in redis and then
				// hitting the server one by one
				jedis.rpush("budgetrental", writeexternal);
			}
		}
		subPageSeeds.clear();
		indivseeds.clear();

		while (jedis.lpop("budgetrental") != null) {
			String d = jedis.lpop("budgetrental");
			Thread.sleep(2000);
		...
		budgetCarRentalsKeyDataSet.clear();
		try {
			SheetLocalWriter.writeXLSXFile("BudgetCarRentalUSA.xlsx", budgetfinalMasterData);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
