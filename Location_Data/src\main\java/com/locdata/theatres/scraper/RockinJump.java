	...
	public static void main(String args[]) throws IOException {
		...
		for (Element r : e) {
			for (Element t : r.select("li")) {
				...
				href = href.endsWith("/") ? href.concat("trampoline-park-contact-us/")
						: href.concat("/trampoline-park-contact-us/");
				locations.add(href);
				counter.addAndGet(1);
			}
		}
		...
		locations.forEach((z) -> {
			...
			// Skip the iteration in chinese website:
			// http://www.obi-rockinjump.com/
			if (document == null)
				return;
			...
			if (rx.get(1).select("h2").text().equalsIgnoreCase("Address")) {
				...
				String rate = rawText.substring(rawText.indexOf("Address") + "Address".length(),
						rawText.indexOf("JOIN OUR MAILING LIST") != -1 ? rawText.indexOf("JOIN OUR MAILING LIST")
				...
				if (z.equalsIgnoreCase("http://westerville.rockinjump.com/trampoline-park-contact-us/")) {

					String addresses = rate.substring(index + "Trampoline Park".length(),
							rate.indexOf("Toll Free Number:"));
					responsebean.setAddress(addresses);
					System.out.println(" Address  =================> " + addresses);

					String phones = rate.split("Toll Free Number:")[1].split("Email:")[0];
					responsebean.setPhoneNumber(phones);
					...
					responsebean.setPlaceId(email);
					System.out.println(" Email  =================> " + email);
				}

				try {
				    new GeoCodingApi(responsebean.getAddress(), responsebean);
				    rockinJumpKeyDataSet.add(responsebean);
				} catch (Exception ex) {
					ex.printStackTrace();
				}
			}
		});
		SheetLocalWriter.writeXLSXFile("RockinJumpLocations.xlsx", rockinJumpKeyDataSet);
	}
}
