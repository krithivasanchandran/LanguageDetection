...
public class PumpItUpPartyLocations {
	...
	public static Set<GeoCodeEntityCarrierToExcelWriter> pumpitupKeyDataSet = new HashSet<GeoCodeEntityCarrierToExcelWriter>();
	...
	public static void main(String args[]) throws IOException{
		...
		locations.forEach((x) -> {
		    ...
		    String storename = document.select("h4[itemprop=name]").text();
		    System.out.println(" Phone : -------> " + phone);
		    ...
		    try {
			    ...
			    pumpitupKeyDataSet.add(responsebean);
			} catch (Exception ex) {
				ex.printStackTrace();
			}
		});
		SheetLocalWriter.writeXLSXFile("PumpItUpPartyLocations.xlsx", pumpitupKeyDataSet);
	}
}